...
bc. def show(id: Long) {
	val post = Posts.(id).getOrNotFound
	Template(post)
}
...
...
def previous(): Post = {
    Posts.find("postedAt < ? order by postedAt desc", postedAt).first.orNull
}
 
def next(): Post = {
    Posts.find("postedAt > ? order by postedAt asc", postedAt).first.orNull
}
